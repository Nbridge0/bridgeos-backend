...
def answer_subject_spending_from_context(
    ...
    Safety:
    - Each extracted item is mapped back to its real document.
    - An unrelated source can never be displayed.
    - Currency is read only from the verified source document.
    - LLM-provided currency and source numbers are not trusted.
    - Decimal performs every calculation.
    """
        ...
        [
            {
                ...
                "amount": str(item.get("amount")),
                "llm_currency": item.get("currency"),
                "llm_source_number": item.get(
                    "source_number"
                ),
                "evidence": item.get("evidence")
            }
    ...
    # =========================================================
    # VERIFY THE REAL SOURCE FOR EVERY EXTRACTED ITEM
    # =========================================================
    verified_items = []

    for item in extracted_items:
        source_number, source_row = (
            resolve_subject_item_source(
                item=item,
                subject=subject,
                matched_rows=matched_rows
            )
        ...
        if source_number is None or source_row is None:
            print(
                "SUBJECT ITEM REJECTED: source could not be verified",
                {
                    "subject": subject,
                    "description": item.get("description"),
                    "evidence": item.get("evidence"),
        ...
        source_text = str(
            source_row.get("search_text")
            or source_row.get("content")
            or source_row.get("text")
            or ""
        )

        # Ignore the LLM-provided currency.
        # Currency is accepted only when found in the verified document.
        verified_currency = determine_currency_from_document(
            source_text
        )

        verified_item = dict(item)
        verified_item["source_number"] = source_number
        verified_item["source_row"] = source_row
        verified_item["currency"] = (
            verified_currency or ""
        )
    ...
    extracted_items = verified_items

    if not extracted_items:
        return {
            "answer": (
                f"I found possible rows for {subject}, but none could "
                "be verified against the correct source document."
            ),
            "sources": []
        }

    # =========================================================
    # DEDUPLICATE COPIES FROM OCR AND EXTRACTED TEXT
    # =========================================================
    unique_items = []
    seen_items = set()
    ...
    for item in extracted_items:
        ...
        # Price x quantity is authoritative when both columns exist.
        ...
        elif isinstance(explicit_total, Decimal):
            final_amount = explicit_total.quantize(
                Decimal("0.01"),
                rounding=ROUND_HALF_UP
            )
            amount_method = "explicit_line_total"

        elif isinstance(current_amount, Decimal):
            final_amount = current_amount.quantize(
                Decimal("0.01"),
                rounding=ROUND_HALF_UP
            )
            amount_method = (
        ...
        item["amount_method"] = amount_method
    ...
    if not extracted_items:
        return {
            "answer": (
                f"I found the {subject} row, but its numeric values "
    ...
    # Do not attach one document's currency to another document.
    if len(known_currencies) > 1:
        ...
        for currency in sorted(known_currencies):
            currency_items = [
                item
                for item in extracted_items
            ...
            for item in currency_items:
                ...
                if (
                    ...
                    answer_lines.append(
                        f"- {item['description']}: "
                        f"{format_currency_amount(unit_price, currency)} "
        ...
        for item in extracted_items:
            ...
            if source_row and source_row not in relevant_rows:
                relevant_rows.append(source_row)
    ...
    if len(known_currencies) == 1 and not has_unknown_currency:
    ...
    # =========================================================
    # SHOW EACH VERIFIED DOCUMENT CALCULATION
    # =========================================================
    for item in extracted_items:
        unit_price = item.get("unit_price")
        quantity = item.get("quantity")
        ...
        if (
            ...
            answer_lines.append(
                f"- {document_name} — {item['description']}: "
    ...
    if has_unknown_currency:
        answer_lines.extend([
            "",
            (
                "No verifiable currency was shown for one or more "
                "matching rows, so no currency symbol was added."
            )
        ])

    # =========================================================
    # SOURCES: ONLY DOCUMENTS THAT CONTRIBUTED TO THE ANSWER
    # =========================================================
    relevant_rows = []
    seen_asset_ids = set()
    ...
    print(
        "SUBJECT SPENDING VERIFIED RESULT:",
        {
            "subject": subject,
            "total": str(total),
            "sources": [
...
def chat(
    ...
    - Loads complete financial document text for monetary calculations.
    - Supports arbitrary products, services, suppliers and categories.
    - Keeps separate invoices separate.
    - Uses Decimal-based calculation helpers.
    - Never combines different currencies.
    - Never returns unrelated source cards for financial answers.
    """
    ...
    def document_financial_score(
        text: str,
        file_name: str
    ) -> int:
        """
        Generic financial-document detector.

        No product, supplier or vendor names are hard-coded.
        """

        searchable = normalise_search_text(
            f"{file_name}\n{text}"
        )
        ...
        for marker in strong_markers:
            if normalise_search_text(marker) in searchable:
                score += 4
    ...
    def financial_text_quality_score(
        text: str
    ) -> int:
        """
        Selects one best text representation per asset.

        This prevents extracted text and OCR from representing the same
        invoice twice and causing duplicate calculations.
        """
        ...
        score = min(len(clean_text), 20000) // 100
        score += sum(
            character.isdigit()
        ...
        for marker in table_markers:
            if normalise_search_text(marker) in normalised:
                score += 40

        return score

    def select_best_financial_text(
        extracted_text: str,
        ocr_text: str,
        summary: str
    ) -> tuple[str, str]:
        """
        Returns exactly one main representation of the financial document.

        The summary is used only when no full extracted/OCR text exists.
        """
        ...
        if extracted_text:
            candidates.append({
                "type": "extracted_text",
                "text": extracted_text,
                "score": financial_text_quality_score(
                    extracted_text
                )
            })
    ...
    def get_subject_terms(
        ...
        """
        Converts any requested subject into generic matching terms.

        No products are hard-coded.
        """
    ...
    def row_contains_subject(
        ...
        row_text = normalise_search_text(
            str(
                row.get("search_text")
                or row.get("content")
                or ""
    ...
    def build_verified_financial_sources(
        financial_result: dict,
        financial_rows: list[dict],
        subject: str
    ) -> list[dict]:
        """
        Prevents an unrelated document from being displayed as the source.

        For subject calculations, only documents containing the requested
        subject are eligible as source cards.
        """

        subject_terms = get_subject_terms(
            subject
        )
        ...
        if subject_terms:
            ...
            if subject_rows:
                return build_sources_from_asset_results(
                    subject_rows
                )
        ...
        if result_sources:
            ...
            for source in result_sources:
                ...
                if asset_id in permitted_asset_ids:
    ...
    if financial_query:
        try:
            ...
            print(
                "FINANCIAL SUBJECT LOOKUP ERROR:",
                type(e).__name__,
                str(e)
            )
    ...
    print(
        "LOCAL CHAT DEBUG:",
        {
            ...
            "financial_subject": requested_financial_subject,
            "answer_depth": answer_depth
        }
    )
...
- Do not answer factual, financial, legal, medical or operational
  questions without document evidence.
...
""".strip()
        ...
        # -----------------------------------------------------
            ...
            # =================================================
                ...
                for asset in financial_assets_res.data or []:
                    ...
                    best_text, selected_text_type = (
                        select_best_financial_text(
                            extracted_text=extracted_text,
                            ocr_text=ocr_text,
                            summary=summary
                        )
                    ...
                    full_document_text = "\n\n".join([
                        f"File name: {file_name}",
                        f"Selected document text type: {selected_text_type}",
                        best_text
                    ]).strip()
                    ...
                    financial_score = document_financial_score(
                        text=best_text,
                        file_name=file_name
                    )

                    # A document is included when:
                    # 1. It looks financial; or
                    # 2. It contains the requested subject.
                    #
                    # This allows arbitrary products such as chicken,
                    # while preventing unrelated SOP/manual documents
                    # containing random numbers from entering the calculation.
                    if (
                        financial_score < 3
                        and not contains_requested_subject
                    ):
                        print(
                            "FINANCIAL DOCUMENT EXCLUDED:",
                            {
                    ...
                    direct_financial_rows.append({
                        ...
                        "search_text": full_document_text,
                        "content_type": (
                            "financial_full_document"
                        ),
                        "chunk_index": 0,
                        "detected_date": None,
                        "detected_year": None,
                ...
                # Do not use content deduplication here.
                # Two separate invoices may contain identical rows.
                matched_rows = direct_financial_rows
                ...
                print(
                    "FINANCIAL DIRECT RETRIEVAL DEBUG:",
                    {
                        ...
                        "document_count": len(matched_rows),
                        "documents": [
                            {
                                "source_number": index,
                                "asset_id": row.get("asset_id"),
    ...
    if not context:
        ...
        if financial_result is not None:
            ...
            sources = build_verified_financial_sources(
                financial_result=financial_result,
                financial_rows=matched_rows,
                subject=requested_financial_subject
            )

            print(
                "FINANCIAL FINAL ANSWER DEBUG:",
                {
                    "answer": answer,
                    "subject": requested_financial_subject,
                    "sources": sources
...
- Do not invent facts, values, dates, names or statuses.
- Answer only when the exact question is supported.
- Inspect all supplied rows.
- Evidence must be copied from the selected source.
            ...
            if (
                ...
                if verified_rows:
                    ...
                    for used_source in raw_used_sources:
                        ...
                        if 0 <= source_index < len(matched_rows):
